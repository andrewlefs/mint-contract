...
const changeAdminAction = async(provider:NetworkProvider, ui:UIProvider) => {
    ...
    const curState = await provider.api().provider(minterContract.address).getState();
...
export async function run(provider: NetworkProvider) {
    ...
    do {
        ...
        const contractState = await provider.api().provider(minterAddress).getState();
        contractState.state.type
    ...
    //EQCwzQGsewVZQIW4Sx6o7raoBXnREYNrh-uuzOJu9pvC4VHY
    ...
    let curAdmin = await minterContract.getTotalSupply()
